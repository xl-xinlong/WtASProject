...
public class CircleProgress extends View {
	...
	public void onDraw(Canvas canvas) {
		...
		if (mBackgroundPicture == null) { // 没背景图的话就绘制底色
			canvas.drawArc(mCircleAttribute.mRoundOval, 0, 360, mCircleAttribute.mBRoundPaintsFill,
					mCircleAttribute.mBottomPaint);
		}
		float subRate = (float) mSubCurProgress / mMaxProgress;
		float subSweep = 360 * subRate;
		canvas.drawArc(mCircleAttribute.mRoundOval, mCircleAttribute.mDrawPos, subSweep,
				mCircleAttribute.mBRoundPaintsFill, mCircleAttribute.mSubPaint);
	...
	class CircleAttribute {
		...
		public void autoFix(int w, int h) {
			if (mSidePaintInterval != 0) {
				...
				mRoundOval.set(sl + mPaintWidth / 2, st + mPaintWidth / 2, w - sr - mPaintWidth / 2,
						h - sb - mPaintWidth / 2);
			}
		}
	}
